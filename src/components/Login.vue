<template>
  <div class="login" id="login">
    ...
    <div class="log-bg">
      ...
      <div class="log-text">@游客</div>
    </div>
    <div class="log-email">
      <input type="text" placeholder="username" :class="'log-input' + (username===''?' log-input-empty':'')"
             v-model="username">
  ...
  export default {
    ...
    methods: {
      ...
      toLogin() {
        ...
        this.axios.post('/api/restapi/login', params).then((response) => {
          let expireDays = 1000 * 60 * 60 * 24 * 15;
          if (Object.keys(response.data).length) {
            this.setCookie('session', JSON.stringify(response.data), expireDays);
            let userInfo = { //保存用户信息
